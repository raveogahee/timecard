...
export async function GET(request: NextRequest) {
  try {
    ...
    // 勤務中のレコードを取得（日付に関係なく）
    const { data: workingRecords, error } = await supabase
      .from('attendance')
      .select('*')
    ...
    if (error) {
      return NextResponse.json({ error: '勤務状態の取得に失敗しました' }, { status: 500 })
    }

    const workingRecord = workingRecords && workingRecords.length > 0 ? workingRecords[0] : null
